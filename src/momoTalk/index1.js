const momoTalk = function () {
  ...
  momoTalk.prototype.pageRenderer = function () {
    ...
    continer.querySelectorAll('.left-tab a').forEach((a, i) => {
      a.className = 'tab';
      a.onclick = () => {
        this.user.page = i;
        this.pageRenderer();
    ...
    continer.querySelector('.left-tab a:nth-child(' + (this.user.page + 1) + ')').classList.add('active');
    if (this.user.page == 0) {
  ...
  momoTalk.prototype.renderRoomLists = function () {
    ...
    if (this.people.unread) {
      ...
      let el = this.mainElm.querySelector('.left-tab span#tCount');
      el.style.display = 'block';
      el.innerText = _countTotal;
      el = this.mainElm.querySelector('.room-container .list_header #title');
      el.innerText += ' (' + _countTotal + ')';
    }
